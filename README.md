...
![alt text](БдДля2ГИС.png)

## 5.2. Таблица описания

| Таблица| Описание| Размер записи (байт) | Число записей | Объем   | Нагрузка на запись (QPS, пик) | Нагрузка на чтение (QPS, пик) |
| --- | --- | --- | --- | --- | --- | --- |
| category| Категории организаций| 60| ~1K| ~60 KB  | ~0| ~2000|
| city| Города| 100| ~500| ~50 KB  | ~0| ~2000|
| organization    | Карточки организаций| 300| ~10M| ~3 GB   | ~50| ~2000|
| Point| Филиалы (адрес, координаты) | 220| ~15M| ~3.3 GB | ~100| ~2000|
| user_account| Пользователи| 180| ~20M (≈DAU)| ~3.6 GB | ~50| ~2000|
| review| Отзывы| 180| ~100M| ~18 GB  | ~200| ~2000|
| Thematic layers | Дорожный граф (маршруты)| 90| ~200M| ~18 GB  | ~50| ~1000|
| tile_cache| Кэш тайлов| 70| ~500M| ~35 GB  | ~1000| ~3000|
---
## 5.3. Требования к консистентности

| Тип консистентности| Таблицы| Обоснование|
| --- | --- | --- |
| **Strict Consistency**   | `organization`, `Point`, `user_account`, `category`, `city` | Критичные данные: координаты, адреса, справочники и учетные записи должны быть строго согласованы, иначе возможны ошибки поиска, навигации и авторизации |
| **Eventual Consistency** | `review`, `Thematic layers`, `tile_cache`                   | Допустима задержка обновления: агрегаты (рейтинг), дорожный граф и кэш не требуют мгновенной согласованности                                             |

##5.4.Особенности распределения нагрузки
| Тип шардирования| Таблицы| Ключ| Обоснование|
| --- | --- | --- | --- |
| Географическое| `Point`, `Thematic layers`| `city_id`| Локализация запросов, снижение latency, работа “рядом со мной” |
| По организации| `organization`, `Point`, `review` | `organization_id` | Быстрая загрузка карточек и филиалов без межшардовых запросов  |
| По пользователю| `user_account`, `review`| `user_id`| Равномерное распределение пользовательской нагрузки|
| Пространственное (тайлы) | `tile_cache`| `(z, x, y)`| Масштабируемый кэш, высокая кратность запросов|
