...
|---|---|---|---|
| Параллельная маршрутизация | 1. Граф очень большой /n 2. Поиск маршрута может быть медленным | 1. Запускается от точки А 2. Параллельно запускается от точки В 3. Алгоритмы встречаются по середине 4. Формируется кратчайший путь | 1. Ускорение поиска маршрута 2. Меньше дублирующих вычислений |
...
| MetaTile | Рендеринг тайтлов по отдельности при высокой нагрузке будет сильно нагружать сервер | 1. Сервер рендерит MetaTile (большой участок карты) 2. Режет его на маленькие тайтлы 3. Тайтлы отдаются клиенту | 1. Снижается нагрузка на сервер 2. Ускоряется рендеринг |
