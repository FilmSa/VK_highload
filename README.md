...
| `tile_cache` | **Redis / CDN / S3-совместимое хранилище** | Тайлы карты читаются очень часто, поэтому их выгодно хранить в кэше и раздавать через CDN; для долговременного хранения подходит объектное хранилище |

## 6.4 Шардирование
Обоснование:
Шардирование при текущей нагрузке излишне, но я закладываю его на этапе проектирования для того, чтобы в будущем было проще масштабировать систему
| Таблица                                                                              | СУБД                 | Ключ шардирования    | Обоснование                                         |
| ------------------------------------------------------------------------------------ | -------------------- | -------------------- | --------------------------------------------------- |
| `user_account`                                                                       | PostgreSQL           | `id`                 | Равномерное распределение пользовательской нагрузки |
| `organization`, `address`, `point`, `building_entrance`, `road_node`, `road_segment` | PostgreSQL / PostGIS | `city_id`            | Географическая локализация запросов                 |
| `review`                                                                             | PostgreSQL           | `point_id`           | Все отзывы объекта на одном шарде                   |
| `tile_cache`                                                                         | Redis / CDN / S3     | `(z, x, y, layer)`   | Пространственное разбиение тайлов                   |
...
| Redis               | `go-redis`                        | Работа с кэшем, сессиями, TTL                                          |
...
## 6.6  Балансировка запросов / мультиплексирование подключений

| Компонент  | Инструмент               | Обоснование                               |
| ---------- | ------------------------ | ----------------------------------------- |
| PostgreSQL | PgBouncer                | Пул соединений, уменьшение нагрузки на БД |
| Redis      | Redis Sentinel / Cluster | Отказоустойчивость и масштабирование      |
| API        | Nginx / Envoy / HAProxy  | L7 балансировка запросов                  |
| Тайлы      | CDN                      | Быстрая доставка контента пользователю    |
...
|---|---|---|---|
| Параллельная маршрутизация | 1. Граф очень большой  2. Поиск маршрута может быть медленным | 1. Запускается от точки А  2. Параллельно запускается от точки В  3. Алгоритмы встречаются по середине  4. Формируется кратчайший путь | 1. Ускорение поиска маршрута  2. Меньше дублирующих вычислений |
| Геопоиск с пространственным индексом | 1. Миллионы точек (point)  2. Полный перебор невозможен | 1. Запрос с координатами  2. По координатам отсекает лишние области 3. Возвращаются ближайшие объекты | 1. Быстрый поиск  2. Нет полного сканирования |
| MetaTile | Рендеринг тайтлов по отдельности при высокой нагрузке будет сильно нагружать сервер | 1. Сервер рендерит MetaTile (большой участок карты) 2. Режет его на маленькие тайтлы 3. Тайтлы отдаются клиенту | 1. Снижается нагрузка на сервер 2. Ускоряется рендеринг 3. Избегается дублирование запросов |
