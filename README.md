...
| -------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Параллельная маршрутизация (Bidirectional Search)**    | 1. Очень большой граф дорог (`road_node` ≈ 150M, `road_segment` ≈ 300M)  <br> 2. Медленный поиск кратчайшего пути при классическом обходе        | 1. Поиск запускается одновременно из точки A и точки B:  <br> координаты → ближайшие вершины через индекс `road_node(lat, lon)` (GiST)  <br><br> 2. Два фронта поиска движутся навстречу друг другу:  <br> обход по таблице `road_segment` через поля `start_node_id`, `end_node_id`, вес — `travel_time_s`  <br><br> 3. При встрече формируется полный путь:  <br> используется пересечение visited-множеств, итоговый путь строится через parent-ссылки                                 | 1. Снижение времени поиска (≈ в 2 раза по сравнению с Dijkstra)  <br> 2. Меньше посещённых вершин → меньше обращений к `road_segment`  <br> 3. Снижение CPU нагрузки на `route service` |
| **MetaTile (групповой рендеринг тайлов)**                | 1. Рендеринг каждого тайла отдельно создаёт высокую нагрузку (пиково ~9444 RPS по тайлам)  <br> 2. Артефакты на границах тайлов (дороги/подписи) | 1. Сервер рендерит большую область:  <br> Mapnik делает запросы к PostgreSQL/PostGIS (`road_segment`, `building`, `point`) и рендерит метатайл (например 1024×1024 px)  <br><br> 2. Изображение разрезается на 16 тайлов:  <br> формируются ключи `(z, x, y, layer)` → соответствуют структуре `tile_cache`  <br><br> 3. Тайлы кэшируются и отдаются клиенту:  <br> - сохраняются в S3 (основное хранилище)  <br> - раздаются через CDN  <br> - метаданные могут храниться в `tile_cache` | 1. Меньше операций рендеринга (в k² раз, при k=4 → в 16 раз)  <br> 2. Ускорение генерации тайлов  <br> 3. Отсутствие артефактов на границах                                             |
