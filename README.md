...
| Геопоиск с пространственным индексом | 1. Миллионы точек (point) 2. Полный перебор невозможен | 1. Запрос с координатами 2. По координатам тсекает лишние области 3. Возвращаются ближайшие объекты | 1. Быстрый поиск 2. Нет полного сканирования |
| MetaTile | Рендеринг тайтлов по отдельности при высокой нагрузке будет сильно нагружать сервер | Сервер рендерит MetaTile(большой участок карты) 2. Режет его на маленькие тайтлы 3. Тайтлы отдаются клиенту| 1. Снижается нагрузка на сервер 2. Ускоряется рендеринг |
