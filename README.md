...
| Алгоритм | Проблема | Как работает | Плюсы | CPU / RAM |
|----------|----------|--------------|-------|-----------|
| **Параллельная маршрутизация (Bidirectional Search)** | 1. Очень большой граф дорог  <br> 2. Медленный поиск кратчайшего пути при классическом обходе | 1. Поиск запускается одновременно из точки A и точки B:  <br> координаты -> ближайшие вершины через индекс `road_node(lat, lon)` (GiST)  <br><br> 2. Два фронта поиска движутся навстречу друг другу:  <br> обход по таблице `road_segment` через поля `start_node_id`, `end_node_id`, вес - `travel_time_s`  <br><br> 3. При встрече формируется полный путь:  <br> используется пересечение visited-множеств, итоговый путь строится через parent-ссылки | 1. Снижение времени поиска  <br> 2. Меньше посещённых вершин -> меньше обращений к `road_segment`  <br> 3. Снижение CPU нагрузки на `route service` | **CPU:** пиковая нагрузка маршрутизации составляет **2406 RPS**: 92 RPS построения маршрутов + 2314 RPS фоновых обновлений. Размер графа: `road_node` = 150 млн, `road_segment` = 300 млн, поэтому средняя степень вершины **300 млн / 150 млн = 2**. При двустороннем поиске и обходе **100000–250000 вершин** в каждом направлении число проверок рёбер на запрос равно **2 × 100000–250000 × 2 = 400000–1000000 операций**. Тогда суммарная нагрузка на CPU равна **2406 × 400000–1000000 = 962400000–2406000000 операций/с** **Для обработки требуется 12–24 CPU cores на сервис с учётом запаса под пики и GC. Используется кластер из 5–8 инстансов по 8–16 cores.** <br>**RAM:** при хранении `visited`, `parent`, `distance` для двух фронтов поиска память составляет **2 × 100000–250000 × 56–72 Б = 10.7–34.3 MiB на запрос**, теоретический максимум по всему графу — **8.4–10.8 ГБ** |
| **MetaTile (групповой рендеринг тайлов)** | 1. Рендеринг каждого тайла отдельно создаёт высокую нагрузку  <br> 2. Артефакты на границах тайлов (дороги/подписи) | 1. Сервер рендерит большую область:  <br> Mapnik делает запросы к PostgreSQL/PostGIS (`road_segment`, `building`, `point`) и рендерит метатайл (например 1024×1024 px)  <br><br> 2. Изображение разрезается на 16 тайлов:  <br> формируются ключи `(z, x, y, layer)` -> соответствуют структуре `tile_cache`  <br><br> 3. Тайлы кэшируются и отдаются клиенту:  <br> - сохраняются в S3  <br> - раздаются через CDN  <br> - метаданные в `tile_cache` | 1. Ускорение генерации тайлов  <br> 2. Отсутствие артефактов на границах | **CPU:** пиковая нагрузка тайлов составляет **9444 RPS**. Один метатайл 1024×1024 покрывает **16 обычных тайлов**, поэтому число операций рендера уменьшается до **9444 / 16 = 590 рендеров/с**. При доле cache miss **1% / 5% / 10%** сервер будет выполнять **6 / 30 / 59 рендеров/с** **Для рендеринга достаточно 4–8 CPU cores при использовании CDN. Используется 2–3 сервера по 4–8 cores.** <br>**RAM:** буфер одного тайла 256×256 в формате RGBA занимает **256 × 256 × 4 = 256 KiB**, буфер метатайла 1024×1024 занимает **1024 × 1024 × 4 = 4 MiB**, то есть память на один активный рендер возрастает в **16 раз**  |
...
| Сервис                  | Описание                                                                                                                                                                  |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Map Query Service**   | Выполняет поиск мест и организаций по названию, категории и ключевым словам; обращается к Elasticsearch                                                                   |
| **Routing Service**     | Строит маршруты по дорожному графу, а также обрабатывает **фоновые обновления маршрута (пересчёт при движении пользователя)**; использует `road_node`, `road_segment`     |
| **Auth Service**        | Обрабатывает регистрацию, авторизацию и аутентификацию пользователей; работает с `user_account` выдает JWT токен                                                          |
| **Places Data Service** | Предоставляет данные об организациях, филиалах и отзывах; работает с `organization`, `point`, `review`, формирует данные для индексации в Elasticsearch                   |
| **Tile Render Service** | Генерирует **тайлы карты и тематические слои (traffic, транспорт и др.)**; использует Mapnik, геоданные из PostgreSQL + PostGIS, реализует MetaTile (п.7), сохраняет в S3 |
| **CDN**                 | Раздаёт тайлы карты, **тематические слои**, оффлайн-карты и статические файлы                                                                                             |
| **L4 LVS**              | Выполняет внешнюю транспортную балансировку входящих соединений                                                                                                           |
| **L7 NGINX**            | Выполняет HTTP-балансировку и маршрутизацию запросов                                                                                                                      |
