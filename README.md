...
| **MetaTile (групповой рендеринг тайлов)** | 1. Рендеринг каждого тайла отдельно создаёт высокую нагрузку  <br> 2. Артефакты на границах тайлов (дороги/подписи) | 1. Сервер рендерит большую область:  <br> Mapnik делает запросы к PostgreSQL/PostGIS (`road_segment`, `building`, `point`) и рендерит метатайл (например 1024×1024 px)  <br><br> 2. Изображение разрезается на 16 тайлов:  <br> формируются ключи `(z, x, y, layer)` -> соответствуют структуре `tile_cache`  <br><br> 3. Тайлы кэшируются и отдаются клиенту:  <br> - сохраняются в S3  <br> - раздаются через CDN  <br> - метаданные в `tile_cache` | 1. Ускорение генерации тайлов  <br> 2. Отсутствие артефактов на границах | **CPU:** пиковая нагрузка тайлов составляет **9444 RPS**. Один метатайл покрывает **16 тайлов**, поэтому число операций рендера равно **9444 / 16 = 590 рендеров/с**. При принятой доле cache miss **5%** сервер выполняет **30 рендеров/с**. Для такой нагрузки достаточно **4 CPU cores суммарно**. **Количество процессоров: 1 процессор на 4 ядра.**  <br>**RAM:** **256 × 256 × 4 = 256 KiB**, **1024 × 1024 × 4 = 4 MiB**, рост в **16 раз**. |
# 8 Технологии
