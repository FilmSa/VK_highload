...
| **Средний размер маршрута** | 4–12 КБ | **Оценка** Средний вес GET запроса на сайти |
...
| **Фоновые обновления навигации** | 20 млн × 0.2 × 25 * 8 кб = 0,7 ТБ | 0.0083 | 0.0166 |
...
| **Обновление офлайн-карт** | 20 млн × 0.2 = 4 млн | 45 | 90 |
...
Кратко можно распределить так:

| Сервис          | Тип балансировки  | Почему                                                                                                                                    |
| --------------- | ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| `tiles.2gis.ru` | **Anycast**       | Тайлы карты - статические данные. Anycast направляет пользователя к ближайшему edge-узлу CDN, что уменьшает задержку при загрузке карты.  |
| `api.2gis.ru`   | **Geo-Based DNS** | Поиск и карточки организаций требуют обращения к региональным базам данных, поэтому пользователя лучше направлять в ближайший дата-центр. |
| `route.2gis.ru` | **Geo-Based DNS** | Построение маршрутов - вычислительная задача, выполняемая на сервере. GeoDNS направляет запрос в ближайший региональный кластер.          |
| `cdn.2gis.ru`   | **Anycast**       | Офлайн-пакеты и статические файлы удобно раздавать через CDN-узлы с Anycast для быстрой загрузки.                                         |
| `2gis.ru`       | **Geo-Based DNS** | Основной сайт должен направлять пользователя в ближайший региональный дата-центр для дальнейших запросов к API.                           |

**Короткое правило:**

* **Anycast** лучше для **статических данных и CDN** (тайлы, файлы, изображения).
* **Geo-DNS** лучше для **API и вычислений**, где важна привязка к конкретному дата-центру.

## 3.5 Механизмы регулировки трафика

Для обеспечения стабильной работы сервиса могут использоваться следующие механизмы регулировки трафика:

- **Rate Limiting** - ограничение количества запросов от одного клиента или IP-адреса для защиты API от перегрузки и злоупотреблений.
- **Load Balancing** - распределение запросов между несколькими серверами внутри дата-центра для равномерного использования ресурсов.
- **Caching** - кэширование часто запрашиваемых данных (тайлы карты, карточки организаций) для снижения нагрузки на базу данных.
- **CDN** - доставка статического контента через edge-узлы, что уменьшает нагрузку на основные серверы.
- **DDoS-защита** - фильтрация вредоносного трафика и распределение нагрузки между узлами сети.
