...
| Алгоритм                                              | Проблема                                                                                                            | Как работает                                                                                                                                                                                                                                                                                                                                                                                                                                               | Плюсы                                                                                                        | Алгоритм | Проблема | Как работает | Плюсы | CPU / RAM |
|----------|----------|--------------|-------|-----------|
| **Параллельная маршрутизация (Bidirectional Search)** | 1. Очень большой граф дорог  <br> 2. Медленный поиск кратчайшего пути при классическом обходе | 1. Поиск запускается одновременно из точки A и точки B:  <br> координаты -> ближайшие вершины через индекс `road_node(lat, lon)` (GiST)  <br><br> 2. Два фронта поиска движутся навстречу друг другу:  <br> обход по таблице `road_segment` через поля `start_node_id`, `end_node_id`, вес - `travel_time_s`  <br><br> 3. При встрече формируется полный путь:  <br> используется пересечение visited-множеств, итоговый путь строится через parent-ссылки | 1. Снижение времени поиска  <br> 2. Меньше посещённых вершин -> меньше обращений к `road_segment`  <br> 3. Снижение CPU нагрузки на `route service` | **CPU:**<br>Пик нагрузки маршрутов: 2406 RPS (92 + 2314) :contentReference[oaicite:0]{index=0}  <br>Средняя степень графа: E/V = 300M / 150M = 2  <br>Если один поиск посещает ~100k–250k вершин на направление:  <br>операций ≈ 4N → 400k–1M операций/запрос  <br>Итого: 2406 × (400k–1M) ≈ **0.96–2.4 млрд операций/с**  <br>→ снижение ~в 2 раза за счёт уменьшения глубины поиска (N → N/2)  <br><br>**RAM:**<br>Память на вершину ≈ 56–72 Б  <br>RAM ≈ 2 × N × M  <br>При N = 100k–250k:  <br>→ **10.7–34.3 MiB на запрос**  <br>Теоретический максимум (V = 150 млн):  <br>→ **8.4–10.8 ГБ**, worst-case до ~21 ГБ |
| **MetaTile (групповой рендеринг тайлов)** | 1. Рендеринг каждого тайла отдельно создаёт высокую нагрузку  <br> 2. Артефакты на границах тайлов (дороги/подписи) | 1. Сервер рендерит большую область:  <br> Mapnik делает запросы к PostgreSQL/PostGIS (`road_segment`, `building`, `point`) и рендерит метатайл (например 1024×1024 px)  <br><br> 2. Изображение разрезается на 16 тайлов:  <br> формируются ключи `(z, x, y, layer)` -> соответствуют структуре `tile_cache`  <br><br> 3. Тайлы кэшируются и отдаются клиенту:  <br> - сохраняются в S3  <br> - раздаются через CDN  <br> - метаданные в `tile_cache` | 1. Ускорение генерации тайлов  <br> 2. Отсутствие артефактов на границах | **CPU:**<br>Пик запросов тайлов: 9444 RPS :contentReference[oaicite:2]{index=2}  <br>MetaTile = 16 тайлов → 9444 / 16 ≈ **590 рендеров/с (100% miss)**  <br>С учётом cache:  <br>1% miss → ~6 рендеров/с  <br>5% miss → ~30 рендеров/с  <br>10% miss → ~59 рендеров/с  <br>→ снижение CPU до **16 раз** (меньше вызовов Mapnik и SQL)  <br><br>**RAM:**<br>256×256×4 = 256 KiB  <br>1024×1024×4 = 4 MiB  <br>→ рост **в 16 раз** (точный расчёт по пикселям)  <br>Практически: **8–12 MiB на worker**, максимум ~16 MiB |
# 8 Технологии
