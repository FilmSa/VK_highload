...
| **Routing Service**     | Строит маршруты по дорожному графу; использует данные `road_node` и `road_segment` из PostgreSQL + PostGIS                                                                |
| **Auth Service**        | Обрабатывает регистрацию, авторизацию и аутентификацию пользователей; работает с таблицей `user_account`                                                                  |
| **Places Data Service** | Предоставляет данные об организациях, филиалах и отзывах; работает с таблицами `organization`, `point`, `review`, а также формирует данные для индексации в Elasticsearch |
| **Tile Render Service** | Генерирует тайлы карты; использует Mapnik, геоданные из PostgreSQL + PostGIS, реализует алгоритм MetaTile из пункта 7, сохраняет тайлы в S3                               |
| **CDN**                 | Раздаёт пользователю тайлы, оффлайн-карты и статические файлы, снижая нагрузку на backend                                                                                 |
| **L4 LVS**              | Выполняет внешнюю транспортную балансировку входящих соединений между L7-узлами                                                                                           |
| **L7 NGINX**            | Выполняет внутреннюю HTTP-балансировку, TLS termination и маршрутизацию запросов к backend-сервисам                                                                       |
| **Prometheus**          | Собирает и хранит метрики сервисов и балансировщиков                                                                                                                      |
| **Grafana**             | Визуализирует метрики из Prometheus в виде графиков и dashboard-панелей                                                                                                   |
